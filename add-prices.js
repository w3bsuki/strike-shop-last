export default async function ({ container }) {
  ...
  try {
    ...
    const defaultRegion =
      regions.find((r) => r.currency_code === 'gbp') || regions[0];
    console.log(
      `💰 Using region: ${defaultRegion.name} (${defaultRegion.currency_code.toUpperCase()})`
    );
    ...
    for (const product of products) {
      ...
      for (const variant of product.variants) {
        ...
        const hasGBPPrice = existingPrices.some(
          (p) => p.currency_code === 'gbp'
        );
        ...
        try {
          ...
          console.log(
            `   💰 ${variant.title} - Added £${(basePrice / 100).toFixed(2)}`
          );
          updatedCount++;
        } catch (error) {
