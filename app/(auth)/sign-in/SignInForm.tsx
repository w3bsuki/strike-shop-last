...
export function SignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Also login to Shopify to get customer access token
      try {
        const shopifyResponse = await fetch('/api/auth/shopify/login', {
          method: 'POST',
        ...
        if (!shopifyResponse.ok) {
          console.log('Shopify sync failed, but user is logged in');
        }
      } catch (shopifyError) {
        // Shopify sync is optional, don't block login
          ...
          <div className="mt-6">
            ...
            <div className="text-center mt-4">
              <Link 
                ...
                className="text-sm text-info-foreground hover:text-info"
              >
                Forgot your password?
              </Link>
            </div>
