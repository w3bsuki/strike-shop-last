...
export function StrikeHeroCarousel({ 
  ...
  // Touch handlers for mobile swipe - OPTIMIZED to prevent scroll conflicts
  const handleTouchStart = (e: React.TouchEvent) => {
    // Only handle horizontal swipes, allow vertical scrolling
    if (window.innerWidth >= 768) return; // Desktop uses arrow buttons
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const distance = Math.abs((touchStart || 0) - currentTouch);
    if (distance > 10) {
      const isHorizontal = Math.abs((touchStart || 0) - currentTouch) > Math.abs((e.targetTouches[0]?.clientY || 0) - (touchStart || 0));
      if (isHorizontal) {
        e.preventDefault();
      }
    }
  ...
  return (
    <div className="relative w-full bg-white overflow-hidden flex flex-col">
      {/* Hero Carousel - MOBILE OPTIMIZED height and touch handling */}
      <section
        className="hero-section relative w-full h-[50vh] md:h-[70vh] lg:h-[80vh] overflow-hidden bg-white flex-shrink-0"
      ...
      >
        ...
        {slides.length > 1 && (
          <>
            <button
              onClick={prevSlide}
              className="hidden md:flex absolute left-4 lg:left-8 top-1/2 -translate-y-1/2 items-center justify-center w-12 h-12 bg-black/10 backdrop-blur-sm border border-black/20 hover:bg-black hover:text-white transition-all duration-300 group rounded-lg"
            ...
            <button
              onClick={nextSlide}
              className="hidden md:flex absolute right-4 lg:right-8 top-1/2 -translate-y-1/2 items-center justify-center w-12 h-12 bg-black/10 backdrop-blur-sm border border-black/20 hover:bg-black hover:text-white transition-all duration-300 group rounded-lg"
          ...
          <div className="absolute bottom-8 md:bottom-12 lg:bottom-16 left-1/2 -translate-x-1/2">
            <Link
              ...
              className={cn(
                ...
                "border-2 min-w-[200px] rounded-lg",
