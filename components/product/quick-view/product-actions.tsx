...
export function ProductActions({
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {sizes.map((size) => (
            <button
              ...
              disabled={isSoldOut}
              className={`py-3 text-xs font-bold uppercase tracking-wider transition-all border focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ${
                selectedSize === size
      ...
      {/* Quantity Selector */}
      <div>
        <h3 className="text-xs font-bold uppercase tracking-wider mb-3">"QUANTITY"</h3>
        <div className="flex items-center space-x-4">
          <button
            onClick={decrementQuantity}
            disabled={isSoldOut}
            className="p-2 hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
      ...
      {!isSoldOut && !selectedSize && (
        <p className="text-[10px] text-center text-[var(--subtle-text-color)]">
