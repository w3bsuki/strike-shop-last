...
function flattenProduct(product: IntegratedProduct) {
  ...
  const currencySymbol = currency === 'BGN' ? 'BGN ' : '€';
  ...
  return {
    ...
    salePrice: product.pricing?.salePrice ? `${currencySymbol}${product.pricing.salePrice}` : undefined,
    image: mainImageUrl, // Simple string URL for ProductCard
    images: product.content?.images?.map(img => typeof img === 'string' ? img : img.url) || [],
